...
## Prototipo actual - [`Versiones/0.1.0/Actual/`](Versiones/0.1.0/Actual/)

Dentro de la carpeta [`Versiones/0.1.0/Actual/`](Versiones/0.1.0/Actual/) se encuentra el proyecto en su estado actual con las siguientes funcionalidades:
...
- Se ejecuta pasando como parametro el archivo .svg que se desea visualizar y se lee el archivo usando SkiaSharp.Svg (el cual ha sido importado en el proyecto).
  Constructor de `MainPage`. Fichero [`Versiones/0.1.0/Actual//Actual/MainPage.xaml.cs`](Versiones/0.1.0/Actual/Actual/MainPage.xaml.cs):
  ```cs
  public MainPage()
  {
...
- Se renderiza la imagen dentro de un SKXamlCanvas usando la funcion PaintSurface que se ejecuta cada vez que se refresca el canvas:
  Insertamos en el archivo [`Versiones/0.1.0/Actual/Actual/MainPage.xaml`](Versiones/0.1.0/Actual/Actual/MainPage.xaml) el canvas:
  ```xaml
  ...
  ```
  Y creamos la funcion `OnPaintSurface` en el archivo [`Versiones/0.1.0/Actual/Actual/MainPage.xaml.cs`](Versiones/0.1.0/Actual/Actual/MainPage.xaml.cs):
  ```cs
...
- Se hace zoom gracias al codigo anterior y a la funcion `PointerWheelChanged` del `SKXamlCanvas`:
  Actualizamos en el archivo [`Versiones/0.1.0/Actual/Actual/MainPage.xaml`](Versiones/0.1.0/Actual/Actual/MainPage.xaml) el `SKXamlCanvas`:
  ```
  <sk:SKXamlCanvas x:Name="canvas"
  ...
  ```
  Y añadimos la funcion `OnSurfacePointerWheelChanged` en el archivo [`Versiones/0.1.0/Actual/Actual/MainPage.xaml.cs`](Versiones/0.1.0/Actual/Actual/MainPage.xaml.cs):
  ```cs
...
- Se mueve la imagen aprentando y arrastrando gracias al uso de la funcion `PointerPressed`, `PointerMoved` y `PointerReleased` del `SKXamlCanvas`:
  ...
  <sk:SKXamlCanvas x:Name="canvas"
                             ...
                             PointerWheelChanged="OnSurfacePointerWheelChanged" />
  ```
  Y añadimos las funciones `OnSurfacePointerMoved`, `OnSurfacePointerPressed` y `OnSurfacePointerReleased` en el archivo [`Versiones/0.1.0/Actual/Actual/MainPage.xaml.cs`](Versiones/0.1.0/Actual/Actual/MainPage.xaml.cs):
  ```cs
...
## Diseño esperado - [`Versiones/0.1.0/Expected`](Versiones/0.1.0/Expected/)
...
- Dentro de la carpeta [`Versiones/0.1.0/Expected/Expected/Presentation`](Versiones/0.1.0/Expected/Expected/Presentation) tenemos el [`MainModel`](Versiones/0.1.0/Expected/Expected/Presentation/MainModel.cs).
...
- Dentro de la carpeta [`Versiones/0.1.0/Expected/Expected/Presentation`](Versiones/0.1.0/Expected/Expected/Presentation) tenemos el MainPage.
  En el archivo [`Versiones/0.1.0/Expected/Expected/Presentation/MainPage.xaml.cs`](Versiones/0.1.0/Expected/Expected/Presentation/MainPage.xaml.cs) vinculamos el DataContext al modelo MainViewModel generado a partir de [MainModel](Versiones/0.1.0/Expected/Expected/Presentation/MainModel.cs):
  ```cs
  ...
  ```
  Y en el archivo [`Versiones/0.1.0/Expected/Expected/Presentation/MainPage.xaml`](Versiones/0.1.0/Expected/Expected/Presentation/MainPage.xaml) añadimos un FeedView el cual estara vinculado a nuestra propiedad:
  ```
  ...
  ```
- El servicio [`Versiones/0.1.0/Expected/Expected/Services/WeatherService.cs`](Versiones/0.1.0/Expected/Expected/Services/WeatherService.cs) es el encargado de suministrar los datos que en este caso son de tipo WeatherInfo:
  ```cs
